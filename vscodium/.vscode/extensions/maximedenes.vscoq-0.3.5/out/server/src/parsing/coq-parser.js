...
VernacControl
 = "Fail" / "Time" / "Redirect" _ String / "Timeout" _ Integer
...
Zs = [\u0020\u00A0\u1680\u2000-\u200A\u202F\u205F\u3000]
`;
const sentenceParser = peg.generate(exports.coqGrammar, { output: "parser", allowedStartRules: ["SentenceLength", "TrySentence"] });
function locationRangeToRange(loc) {
    return vscode_languageserver_1.Range.create(loc.start.line - 1, loc.start.column - 1, loc.end.line - 1, loc.end.column - 1);
}
exports.locationRangeToRange = locationRangeToRange;
class SyntaxError extends Error {
    constructor(name, message, range, startOffset, endOffset, found, expected, stack) {
        super(message);
        this.name = name;
        this.range = range;
    ...
    static unknown(parseStr) {
        const range = vscode_languageserver_1.Range.create(textUtil.positionAt(parseStr, 0), textUtil.positionAt(parseStr, parseStr.length));
        let result = new SyntaxError("unknown", "Unexpected parse error", range, 0, parseStr.length);
        return result;
    }
}
exports.SyntaxError = SyntaxError;
function parseSentenceLength(str) {
    try {
        sentenceParser.parse(str, { startRule: "SentenceLength", tracer: undefined });
        return -1;
    }
    catch (error) {
...
exports.parseSentenceLength = parseSentenceLength;
function parseSentence(str) {
    try {
        return sentenceParser.parse(str, { startRule: "TrySentence", tracer: undefined });
    }
    catch (error) {
...
const skipCommentRE = /^(?:[^*(]|\*(?!\))|\((?!\*))*(\*\)|\(\*)|$/;
function doSimpleSkip(str, idx, re) {
...
// }
function doSkipComment(str, idx) {
 ...
 * @returns `false` if the edit might change the validity of the sentence and thus needs to be reinterpreted
 */
function isPassiveDifference(cmd1, cmd2) {
    try {
...
exports.isPassiveDifference = isPassiveDifference;
var SentenceRangeContainment;
(function (SentenceRangeContainment) {
    ...
    SentenceRangeContainment[SentenceRangeContainment["Contains"] = 0] = "Contains";
    /** The range is before the sentence. Adjust sentence position */
    SentenceRangeContainment[SentenceRangeContainment["Before"] = 1] = "Before";
    /** The range is after the sentence. Ignore change. */
    SentenceRangeContainment[SentenceRangeContainment["After"] = 2] = "After";
    /** The change crosses the sentence boundary. Invalidate sentence. */
    SentenceRangeContainment[SentenceRangeContainment["Crosses"] = 3] = "Crosses";
})(SentenceRangeContainment = exports.SentenceRangeContainment || (exports.SentenceRangeContainment = {}));
...
*/
function sentenceRangeContainment(sentRange, range) {
...
exports.sentenceRangeContainment = sentenceRangeContainment;
