...
class CoqDocument {
    ...
    onCoqMessage(level, message, routeId) {
    ...
    resetCoq() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.stm = new STM_1.CoqStateMachine(this.project, () => {
                ...
                coqDied: (reason, error) => this.onCoqDied(reason, error),
            });
        });
    }
    ...
    *commandSequenceGenerator(begin, end, highlight = false) {
    ...
    toGoal(goal) {
        ...
        return Object.assign({ focus: this.stm.getFocusedPosition() }, goal);
    ...
    updateDiagnostics(now = false) {
        ...
        this.feedback.updateDiagnostics(() => {
            ...
            for (let d of this.stm.getDiagnostics()) {
                var range = d.sentence;
                if (d.range) {
                    range = d.range;
                }
                diagnostics.push(vscode_languageserver_1.Diagnostic.create(range, AnnotatedText_1.textToDisplayString(d.message), d.severity, undefined, 'coqtop'));
            }
    ...
    getCachedGoal(pos, direction) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.isStmRunning())
                return { type: 'not-running', reason: "not-started" };
            try {
                return this.toGoal(yield this.stm.getCachedGoal(pos, direction));
            }
            finally {
                this.updateDiagnostics(true);
            }
    ...
    query(query, term, routeId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (query) {
                case "locate":
                    try {
                        return yield this.stm.doQuery(`Locate ${term}.`, routeId);
                    }
                    catch (err) {
                        return yield this.stm.doQuery(`Locate "${term}".`, routeId);
                    }
                case "check": return yield this.stm.doQuery(`Check ${term}.`, routeId);
                case "print": return yield this.stm.doQuery(`Print ${term}.`, routeId);
                case "search": return yield this.stm.doQuery(`Search ${term}.`, routeId);
                case "about": return yield this.stm.doQuery(`About ${term}.`, routeId);
                case "searchAbout": return yield this.stm.doQuery(`SearchAbout ${term}.`, routeId);
            }
        });
    }
